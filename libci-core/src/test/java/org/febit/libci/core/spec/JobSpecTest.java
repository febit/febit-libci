...
class JobSpecTest {
    ...
    class Deserialization {
        ...
        void releaseAndTrigger() {
            ...
            assertEquals(JobSpec.ReleaseAssetLinkType.RUNBOOK,
                    spec.release().assets().links().getFirst().linkType());
    ...
    class Merge {
        ...
        @Test
        void withNullNeeds() {
            var raw = YamlUtils.loader().source(jobYaml("merge-test", """
                    stage: deploy
                    script:
                      - echo hello
                    """)).load();
            ...
            var rule = JobSpec.Rule.builder().needs(null).build();
            assertNull(rule.needs());

            var merged = spec.merge(rule);
            assertNotNull(merged);
        }

        @Test
        void withNullVariables() {
            var raw = YamlUtils.loader().source(jobYaml("merge-test-2", """
                    stage: deploy
                    script:
                      - echo hello
                    variables:
            ...
            var rule = JobSpec.Rule.builder().variables(null).build();
            assertNull(rule.variables());

            var merged = spec.merge(rule);
            assertNotNull(merged);
            assertEquals("bar", merged.variables().get("FOO").value());
        }
    }
}
